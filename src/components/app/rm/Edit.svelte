<script>
  ...
  async function load_rm (id) {
    if (!id) throw new Error(`id invalid: ${id}`);
    try {
      const response = await http.get(`/v1/api/rm/${id}`);
      const item = response.data;
      const server_date = new Date(item.waktu);
      tanggal = format(server_date, "yyy-MM-dd");
      waktu = format(server_date, "HH:mm");
      nama_pasien = item.nama_pasien;
      solusi = item.solusi;
      gejala = item.gejala.map(g => g.id);
      status = item.status;
      id_penyakit = item.id_penyakit;
    } catch (err) {
      console.log(err);
      alert("gagal mengambil data rekam medik");
    }
  }
...
<div class="font-black text-center text-3xl mb-10">Tambah Data Rekam Medik</div>
