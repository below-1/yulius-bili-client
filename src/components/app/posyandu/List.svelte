<script>
  ...
  async function deleteItem (id) {
    try {
      const url = `/api/posyandu/delete.php?id=${id}`;
  ...
  async function loadData () {
    try {
      const response = await http.get('/api/posyandu/list.php');
      items = response.data;
      console.log(items);
    } catch (err) {
      alert('gagal');
...
<div class="rounded bg-gray-200 p-4 flex items-center">
  <h2 class="font-black text-xl">Daftar Posyandu</h2>
