<script>
  ...
  async function save () {
    ...
    const url = '/api/bahan_pangan/update.php';
    try {
      const result = await http.post(url, payload);
      console.log(result);
      alert('sukses mengubah data bahan pangan');
      pop();
    } catch (err) {
      console.log(err);
      errorMessage = 'gagal mengubah data bahan pangan';
    }
  }

  async function getBahanPangan ({ id }) {
    if (!id) return;
    const url = `/api/bahan_pangan/single.php?id=${id}`;
    try {
      const response = await http.get(url);
      const item = response.data;
      nama = item.nama;
      kategori = item.kategori;
      berat_min = item.berat_min;
      berat_max = item.berat_max;
      keb_energi = item.keb_energi;
      keb_protein = item.keb_protein;
      keb_lemak = item.keb_lemak;
      keb_karbohidrat = item.keb_karbohidrat;
      satuan_urt = item.satuan_urt;
      nilai_urt = item.nilai_urt;
      errorMessage = null;
    } catch (err) {
      console.log(err);
...
<div class="font-black text-center text-3xl mb-10">Edit Data Bahan Pangan</div>
