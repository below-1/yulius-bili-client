<script>
  ...
  async function save () {
    ...
    const url = '/v1/api/gejala';
    try {
      const result = await http.post(url, payload);
      console.log(result.data);
      alert('sukses menambah data gejala');
      push('/app/gejala');
    } catch (err) {
      console.log(err);
      errorMessage = 'gagal menambah data gejala';
      return;
    }
  }
</script>

<div class="font-black text-center text-3xl mb-10">Tambah Data Gejala</div>
