<script>
  ...
  const schema = object().shape({
    ...
    bb: number().required('berat badan harus diisi').positive('berat badan harus lebih besar dari 0'),
    tb: number().required('tinggi badan harus diisi').positive('tinggi badan harus lebih besar dari 0')
  });
  ...
  async function getOptionsPosyandu() {
    ...
    try {
      ...
      const optionsPosyandu = response.data.map(posyandu => ({
        value: posyandu.id,
        text: posyandu.nama
      }));
      return optionsPosyandu;
    } catch (err) {
  ...
  async function save () {
    ...
    const url = `/api/balita/update.php?id=${id}`;
    try {
      ...
      console.log(err);
      errorMessage = 'gagal mengubah data balita';
      alert(errorMessage);
    }
  }
  ...
  onMount(async () => {
    try {
      optionsPosyandu = await getOptionsPosyandu();
    } catch (err) {
      console.log(err);
...
<div class="font-black text-center text-3xl mb-10">Edit Data Balita</div>
