<script>
  ...
  import { GiziBee,  } from '../../../services/bee_uji';
  ...
  let balita = {};
  let nSolutions = 0;
  let ALL_KATEGORI = ['karbohidrat', 'protein', 'lemak'];
  let bee = null;
  let solution = null;
  let solutions = [];
  let listDetails = [];
  ...
  onMount(async () => {
    alert("here");
    console.log("uji generator");
    if (!params.id) return;
    const idBalita = parseInt(params.id);

    nSolutions = parseInt(localStorage.getItem("sigizi.uji.nSolution"));
    ALL_KATEGORI = JSON.parse(localStorage.getItem("sigizi.uji.kategoris"));
